...
// todo: tests /activatelogin and /myCash

const testEntityValue = (entity, value) => {
  expect(entity).to.be.an('object')
  expect(entity).to.contain.all.keys(['_id', 'updated_at', 'created_at', 'personalDocument', 'name', 'email', 'username', 'active'])
...
describe(ENDPOINT, () => {
  ...
  before(async () => {
    authorizedUser = await Model.create({username: 'test0', name: 'test', email: 'test0@test.com', password: 'test', personalDocument: '0987654321', active: true})
  ...
  describe('POST /', () => {
    ...
    it('should validate invalid params', async () => {
      ...
      const userData = {email: '2--01', password: 'abcde', username: 'abc', personalDocument: '12345', active: 'NOT A BOOLEAN', name: 'asd'}
    ...
    it('should validate username uniqueness', async () => {
      ...
      const userData = {email: 'tes201@test.com', password: '111abcde', username: 'test0', personalDocument: '12345665432', name: 'new user'}
    ...
    it('should validate email uniqueness', async () => {
      ...
      const userData = {email: 'test0@test.com', password: '111abcde', username: 'johntravolta', personalDocument: '09876123456', name: 'new user'}
    ...
    it('should respond with success status and the created user when valid data is sent', async () => {
      const userData = {email: 'testing0@gmail.com', password: 'potato', username: 'potatouser0', personalDocument: '84820582374', name: 'test batata'}
      const res = await request(app)
        .post(ENDPOINT)
        .send(userData)
    ...
    it('should create a new record on DB when valid data is sent', async () => {
      ...
      const userData = {email: 'testing1@gmail.com', password: 'potato', username: 'potatouser1', personalDocument: '54365277456', name: 'batata test'}
  ...
  describe('GET /', () => {
    ...
    it('should return valid data', async () => {
      ...
      res.body.users.every(u => {
        ...
        expect(u).to.contain.all.keys(['active', '_id', 'email', 'username', 'personalDocument', 'name', 'created_at', 'updated_at'])
        return true
      })
    })
  })
  ...
  describe('GET /:id', () => {
    ...
    it('should respond a valid found entity', async () => {
      ...
      expect(res.body).to.contain.all.keys(['active', '_id', 'email', 'personalDocument', 'username', 'name', 'created_at', 'updated_at'])
    })
  })
  ...
  describe('PATCH /:id', () => {
    ...
    before(async () => {
      someNewUser = await Model.create({username: 'toupdate', name: 'updated', email: 'test0@update.com', personalDocument: '33333333333', password: 'update', active: true})
    })
    ...
    it('should validate invalid params', async () => {
      ...
      const userData = {email: '2--01', username: 'abc', personalDocument: '4444', name: 'asd'}
  ...
  describe('DELETE /:id', () => {
    ...
    it('should respond with success status and the created user when valid data is sent', async () => {
      const toDeleteUser = await Model.create({username: 'todelete', name: 'will be deleted', personalDocument: '34387483473', email: 'test0@delete.com', password: 'delete', active: true})
      const res = await request(app)
    ...
    it('should remove the matching record on DB when authorized request', async () => {
      const toDeleteUser = await Model.create({username: 'todelete1', name: 'will be deleted', personalDocument: '34387483411', email: 'tes1t0@delete.com', password: 'delete', active: true})
      await request(app)
        .delete(`${ENDPOINT}/${toDeleteUser._id}`)
        .set(authorizedHeader)
        .expect(204)
